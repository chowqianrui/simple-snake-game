...
class SnakeGame {
	...
	step() {
		...
		//detect wall and auto change direction
		if (new_segment.x == 0 && new_segment.y == 0){
			if (this.direction && this.direction.x == 0 && this.direction.y == -1){ // if up go right
				this.direction.x = 1;
				this.direction.y = 0;
			} 
			else if (this.direction && this.direction.x == -1 && this.direction.y == 0){ // if left, go down
				this.direction.x = 0;
				this.direction.y = 1;
			}
		}
		else if (new_segment.x == this.dims[0]-1 && new_segment.y == 0){
			if (this.direction && this.direction.x == 1 && this.direction.y == 0){ // if right go down
				this.direction.x = 0;
				this.direction.y = 1;
			}
			else if (this.direction && this.direction.x == 0 && this.direction.y == -1){ // if up go left
				this.direction.x = -1;
				this.direction.y = 0;
			}
		}
		else if (new_segment.x == 0 && new_segment.y == this.dims[1]-1){
			if (this.direction && this.direction.x == -1 && this.direction.y == 0){ // if left go up
				this.direction.x = 0;
				this.direction.y = -1;
			}
			else if (this.direction && this.direction.x == 0 && this.direction.y == 1){  // if down, go right
				this.direction.x = 1;
				this.direction.y = 0;
			}
		}
		else if (new_segment.x == this.dims[0]-1 && new_segment.y == this.dims[1]-1){
			if (this.direction && this.direction.x == 0 && this.direction.y == 1){ // if down, go left
				this.direction.x = -1;
				this.direction.y = 0;
			}
			else if (this.direction && this.direction.x == 1 && this.direction.y == 0){ // if right go up
				this.direction.x = 0;
				this.direction.y = -1;
			}
		}
		else if (new_segment.y == 0 && this.direction.x == 0 && this.direction.y == -1){ // up wall, go right
			this.direction.x = 1;
			this.direction.y = 0;
		}
		else if (new_segment.x == this.dims[0]-1 && this.direction.x == 1 && this.direction.y == 0){ //right wall, go down
			this.direction.x = 0;
			this.direction.y = 1;
		}
		else if (new_segment.y == this.dims[1]-1 && this.direction.x == 0 && this.direction.y == 1){ //bottom wall, go left
			this.direction.x = -1;
			this.direction.y = 0;
		}
		else if (new_segment.x == 0 && this.direction.x == -1 && this.direction.y == 0){ //left wall, go up
			this.direction.x = 0;
			this.direction.y = -1;
		}
